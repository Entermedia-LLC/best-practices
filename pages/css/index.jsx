...
@media (prefers-reduced-motion: reduce) {
    .animation {
        ...
        <p>
          Let&apos;s be honest, CSS “speed” and performance is not as important
          as PHP or JavaScript performance. However, this doesn&apos;t mean we
          should ignore it. A sum of small improvements equals better experience
          for the user.
        </p>
        <p>
        ...
        <p>
          ...
          libraries such as <code>respond.js</code> as it does not work well in
          certain environments. Instead, we leverage a natural, mobile-first
          build process and allow sites gracefully degrade.
        </p>
        ...
        <ul>
          ...
          <li>
            It targets the least capable browsers first which is philosophically
            in line with mobile first — a concept we often embrace for our sites
          </li>
          <li>
        ...
        <p>
          ...
          can take advantages of reusability and avoid having an unmaintainable
          number of breakpoints. Using variables and reusable code blocks we can
          lighten the CSS load and ease maintainability.
        </p>
        <Heading section="3">Media queries placement</Heading>
        <p>
          In your stylesheet files, nest the media query within the component it
          modifies. <strong>Do not</strong> create size-based partials (e.g.{" "}
          <code>_1024px.(s)css</code>, <code>_480px.(s)css</code>): it will be
          frustrating to hunt for a specific selector through all the files when
          we have to maintain the project. Putting the media query inside the
          component will allow developers to immediately see all the different
...
.some-other-class {
	color: orange;
        ...
        <p>
          ...
          our code style consistent, we not only help ourselves debug faster but
          we&apos;re also lessening the burden on those who will have to
          maintain our code (maybe ourselves too!).
        </p>
        <Heading section="3">CSS Syntax</Heading>
        <p>
          CSS syntax is not strict and will accept a lot of variations, but for
          the sake of legibility and fast debugging, we follow basic code
          styles:
        </p>
        <ul>
...
.class-2 {
  ...
  box-shadow: 0 1px 5px #000, 1px 2px 5px #ccc;
        ...
        <ul>
          ...
          <li>
            End all declarations with a semi-colon, even the last one, to avoid
            errors
          </li>
  ...
  margin: 0;
        ...
        <p>
          If you don&apos;t need to set all the values, don&apos;t use shorthand
          notation.
        </p>
        <p className={classes["error-text"]}>Don&apos;t:</p>
        <pre>
  ...
  margin-left: 10px;
        ...
        <p>
          Declarations should be ordered alphabetically or by type (Positioning,
          Box model, Typography, Visual). Whichever order is chosen, it should
        ...
        <p>
          ...
          more difficult to read and can create confusion. Too much nesting also
          adds unnecessary specificity, forcing us to add the same or greater
          specificity in overrides. We want to avoid selector nesting and
          over-specificity as much as possible.
        </p>
        <p>
	...
	background-color: red;
        ...
        <p>
          ...
          third-party libraries, use vendor prefixes. Don&apos;t use names that
          can be blocked by adblockers (e.g. “advertisement”). When in doubt,
          you can check a class name against{" "}
          <a
        ...
        <p>
          We follow WordPress official commenting standards. Do not hesitate to
          be very verbose with your comments, especially when documenting a
          tricky part of your CSS. Use comment blocks to separate the different
          sections of a partial, and/or to describe what styles the partial
          covers:
        </p>
        <pre>
...
li:nth-child(n+4):nth-child(-n+8) {
	color: red;
        ...
        <ul>
          <li>
            Limit the number of requests by concatenating CSS files and encoding
            sprites and font files to the CSS file.
          </li>
...
.sibling-2 {
	...
	font-family: Arial, sans-serif;
        ...
        Styles that can be shared, should be shared (aka DRY, Don&apos;t Repeat
        Yourself). This will make our stylesheets less bloated and prevent the
        browser from doing the same calculations several times. Make good use of
        Sass placeholders. (also see{" "}
        <Link href="/css/#inheritance">Inheritance</Link>)
        <Heading section="3">CSS over assets</Heading>
        <p>
          Don&apos;t add an extra asset if a design element can be translated in
          the browser using CSS only. We value graceful degradation over
          additional HTTP requests.
        </p>
        <p>Very common examples include gradients and triangles.</p>
        <Heading section="3">Animations</Heading>
        <p>
          It&apos;s a common belief that CSS animations are more performant than
          JavaScript animations. A few articles aimed to set the record straight
          (linked below).
        </p>
        <ul>
          <li>
        ...
        <p>
          Limit your CSS animations to 3D transforms (translate, rotate, scale)
          and opacity, as those are aided by the GPU and thus smoother. Note
          that too much reliance on the GPU can also overload it.
        </p>
        <p className={classes["error-text"]}>Don&apos;t:</p>
        <pre>
...
#menu li:hover {
  left: 10px
        ...
        <p>
          Always test animations on a real mobile device loading real assets, to
          ensure the limited memory environment doesn&apos;t tank the site.{" "}
          <strong>Note:</strong>{" "}
          <a
        ...
        <p>
          Sometimes a more complex grid system is warranted and leveraging a 3rd
          party library will gain some efficiency. However, keep in mind that by
          adopting a grid system you are forcing all future collaborators on the
          project to learn this system.
        </p>
